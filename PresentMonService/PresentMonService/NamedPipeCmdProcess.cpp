...
bool EncodeStartStream(PresentMon* pm, MemBuffer* rqstBuf, MemBuffer* rspBuf) {
  ...
  IPMSMFileName fileName = {};

  if (rspStatus != PM_STATUS::PM_STATUS_SUCCESS) {
    NamedPipeHelper::PopulateResponseHeader(
        response, PM_ACTION::START_STREAM, 1, sizeof(IPMSMFileName), rspStatus);
    rspBuf->AddItem(&response, sizeof(response));
    rspBuf->AddItem(&fileName, sizeof(IPMSMFileName));
    ...
    strcpy_s(fileName.fileName, sizeof(fileName.fileName), nsmFileName.c_str());
    fileName.fileNameLength = nsmFileName.length();
    rspBuf->AddItem(&fileName, sizeof(IPMSMFileName));
  }
