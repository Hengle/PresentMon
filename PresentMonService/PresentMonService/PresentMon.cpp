...
PresentMonSession::PresentMonSession()
  ...
  streaming_started_ = CreateEvent(NULL, TRUE, FALSE, NULL);
}
...
PresentMonSession::~PresentMonSession() {
  ...
  if (pm_session_name_.size() > 0) {
    pm_session_name_.clear();
  }
  if (streaming_started_ != INVALID_HANDLE_VALUE) {
    CloseHandle(streaming_started_);
    streaming_started_ = INVALID_HANDLE_VALUE;
  }
}
...
PM_STATUS PresentMonSession::StartTraceSession() {
  ...
  // Create consumers
  pm_consumer_ = new PMTraceConsumer();
  if (pm_consumer_ == nullptr) {
    PM_STATUS::PM_STATUS_ERROR;
  }
  ...
  auto status = trace_session_.Start(pm_consumer_, nullptr, etl_file_name,
                                     pm_session_name_.c_str());

  if (status == ERROR_ALREADY_EXISTS) {
    status = TraceSession::StopNamedSession(pm_session_name_.c_str());
    if (status == ERROR_SUCCESS) {
      status = trace_session_.Start(pm_consumer_, nullptr, etl_file_name,
                                    pm_session_name_.c_str());
    }
  }
  ...
  if (status != ERROR_SUCCESS) {
    // TODO(megalvan): Add in a control flag that outputs error status to the
...
void PresentMonSession::AddPresents(
  ...
  for (auto n = presentEvents.size(); i < n; ++i) {
    ...
    if ((pm_session_name_.compare(kRealTimeSessionName) == 0) &&
        (current_telemetry_adapter_)) {
      if (auto data = current_telemetry_adapter_->GetClosest(
    ...
    if (result.second) {
      ...
      chain->mLastProducerPresentQPC = 0;
      chain->mLastConsumerPresentQPC = 0;
    }
    ...
    if (chain->mPresentHistoryCount > 0) {
      ...
      streamer_.ProcessPresentEvent(
          ...
          chain->mLastProducerPresentQPC, chain->mLastConsumerPresentQPC,
          processInfo->mModuleName);
    }

    chain->mLastPresentQPC = presentEvent->PresentStartTime;
    // Internal fields; Remove for public build
    chain->mLastProducerPresentQPC = presentEvent->INTC_ProducerPresentTime;
    chain->mLastConsumerPresentQPC = presentEvent->INTC_ConsumerPresentTime;
...
PresentMonSession::EnumerateAdapters() {
  if (all_telemetry_adapters_) {
    return *all_telemetry_adapters_;
  } else {
    return {};
  }
}
...
PM_STATUS PresentMonSession::SelectAdapter(uint32_t adapter_id) {
  if (adapter_id > all_telemetry_adapters_->size()) {
    return PM_STATUS_INVALID_ADAPTER_ID;
  }
  current_telemetry_adapter_ = all_telemetry_adapters_->at(adapter_id).get();
  return PM_STATUS::PM_STATUS_SUCCESS;
}
...
void PresentMon::SetTelemetryAdapters(
    const std::vector<std::shared_ptr<pwr::PowerTelemetryAdapter>>& adapters) {
  // Only the real time trace uses the control libary interface
  real_time_session_.SetTelemetryAdapters(adapters);
}
