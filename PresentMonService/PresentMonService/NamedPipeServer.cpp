...
DWORD NamedPipeServer::CreateNamedPipesAndEvents() {
  for (DWORD i = 0; i < mMaxPipes; i++) {
    ...
    if (mEvents[i] == NULL) {
      mService->ServiceReportEvent(
          TEXT("Unable to create communication event."));
      return GetLastError();
    }
    ...
    if (const auto result = mPipe[i].CreatePipeInstance(
      ...
      mService->ServiceReportEvent(
          TEXT("Unable to create communication object."));
      return result;
    }

    // Call the subroutine to connect to the new client
    DWORD connectionStatus = ConnectToNewClient(
        mPipe[i].mPipeInstance, &mPipe[i].mOverlap, mPipe[i].mPendingIO);
...
DWORD NamedPipeServer::ConnectToNewClient(HANDLE pipe, LPOVERLAPPED overlapped,
  ...
  if (connected == TRUE) {
    mService->ServiceReportEvent(TEXT("Unable to connect to new client."));
    return GetLastError();
  }
...
void NamedPipeServer::GetPendingOperationResult(DWORD pipeIndex) {
  ...
  if (mPipe[pipeIndex].mPendingIO) {
    ...
    BOOL success =
        GetOverlappedResult(mPipe[pipeIndex].mPipeInstance,  // handle to pipe
    ...
    switch (mPipe[pipeIndex].mCurrentState) {
      case PipeStates::CONNECTING:
        ...
        if (!success) {
          mService->ServiceReportEvent(TEXT("Pending connection failed."));
          return;
        }
...
void NamedPipeServer::DisconnectAndReconnect(DWORD pipeIndex) {
  ...
  if (!DisconnectNamedPipe(mPipe[pipeIndex].mPipeInstance)) {
    mService->ServiceReportEvent(TEXT("Disconnect to client failed."));
    return;
  }

  DWORD connectionStatus = ConnectToNewClient(mPipe[pipeIndex].mPipeInstance,
  ...
  if (connectionStatus == ERROR_SUCCESS) {
    ...
    mService->ServiceReportEvent(TEXT("Reconnect to client failed."));
  }
...
void NamedPipeServer::ExecuteCurrentState(DWORD pipeIndex) {
  ...
  switch (mPipe[pipeIndex].mCurrentState) {
    ...
    default:
      mService->ServiceReportEvent(
          TEXT("Unknown execution state encountered."));
      break;
  }
...
void NamedPipeServer::ReadRequestMessage(DWORD pipeIndex) {
  BOOL success = ReadFile(
      mPipe[pipeIndex].mPipeInstance, mPipe[pipeIndex].mPipeReadBuffer,
