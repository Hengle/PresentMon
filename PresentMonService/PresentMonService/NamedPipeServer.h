 ...
 private:
  ...
  class Pipe {
   public:
    ...
    ~Pipe() {
      if (mPipeInstance != INVALID_HANDLE_VALUE) {
        FlushFileBuffers(mPipeInstance);
        DisconnectNamedPipe(mPipeInstance);
        CloseHandle(mPipeInstance);
      }
    }
    ...
    OVERLAPPED mOverlap = {};
    HANDLE mPipeInstance = INVALID_HANDLE_VALUE;
    DWORD mBytesRead = 0;
