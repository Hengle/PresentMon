...
extern "C" {
  ...
  struct PM_FRAME_DATA
  {
    ...
    double ms_waiting_on_queue_sync;
    double ms_waiting_on_queue_drain;
    double ms_waiting_on_fence;
    double ms_waiting_on_fence_submission;
    double ms_stalled_on_queue_empty;
    double ms_between_producer_presents;
    double ms_between_consumer_presents;
    ...
    double ms_waiting_on_create_time_compilation;
    double ms_in_make_resident;
    double ms_in_paging_packets;
    ...
    double vram_effective_frequency_gbs;
    double vram_read_bandwidth_bps;
    double vram_write_bandwidth_bps;
    double vram_temperature_c;

    double fan_speed_rpm[MAX_PM_FAN_COUNT];

    PM_PSU_TYPE psu_type[MAX_PM_PSU_COUNT];
    double psu_power[MAX_PM_PSU_COUNT];
    double psu_voltage[MAX_PM_PSU_COUNT];
    ...
    // Throttling flags
    int gpu_power_limited;
    int gpu_temperature_limited;
    int gpu_current_limited;
    int gpu_voltage_limited;
    int gpu_utilization_limited;

    int vram_power_limited;
    int vram_temperature_limited;
    int vram_current_limited;
    int vram_voltage_limited;
    int vram_utilization_limited;
    ...
    // Bias hints only for internal Intel builds. Do not
    // expose externally
    double cpu_bias;
    double gpu_bias;
  };
  ...
  struct PM_FPS_DATA
  {
    ...
    PM_METRIC_DOUBLE_DATA display_fps;
    PM_METRIC_DOUBLE_DATA cpu_fps;
    double gpu_fps_avg;

    uint32_t num_presents;
    uint32_t num_dropped_frames;
