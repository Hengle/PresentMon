...
bool NamedPipeHelper::ValidatePayloadRequestSize(PM_ACTION action,
  ...
  switch (action) {
    ...
    case PM_ACTION::ENUMERATE_ADAPTERS: {
      // Enumeration of GPUs only requires the request header.
...
bool NamedPipeHelper::ValidatePayloadResponseSize(PM_ACTION action,
  ...
  switch (action) {
    case PM_ACTION::START_STREAM: {
      ...
      // IPMSMFileName struct Check the header value
      if (header_payload_size != sizeof(IPMSMFileName)) {
        return false;
      }
...
PM_STATUS NamedPipeHelper::DecodeStartStreamingResponse(
    MemBuffer* rqst_buf, IPMSMFileName* mapfile_name) {
  ...
  if (response->result == PM_STATUS::PM_STATUS_SUCCESS) {
    memcpy_s(mapfile_name, sizeof(IPMSMFileName),
             GetResponsePayloadPtr(rqst_buf, PM_ACTION::START_STREAM),
             sizeof(IPMSMFileName));
  }
...
PM_STATUS NamedPipeHelper::EncodeEnumerateAdaptersRequest(MemBuffer* rqst_buf) {
  IPMSMRequestHeader request;

  PopulateRequestHeader(request, PM_ACTION::ENUMERATE_ADAPTERS, 1, 0);
  rqst_buf->AddItem(&request, sizeof(request));
  return PM_STATUS::PM_STATUS_SUCCESS;
}
