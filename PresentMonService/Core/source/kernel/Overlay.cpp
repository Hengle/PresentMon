...
namespace p2c::kern
{
            ...
            for (const auto& w : spec.widgets) {
                try {
                    if (auto pGraphSpec = std::get_if<GraphSpec>(&w)) {
                        auto packsData = pGraphSpec->metricIndices |
                            std::views::transform([&](size_t iMetric) {
                                auto& pack = graphPacks.at(iMetric);
                        ...
                        pReadoutContainer->AddChild(gfx::lay::ReadoutElement::Make(
                            ...
                            pack.pMetric->GetUnits(), &pack.text)
                        );
                    }
                    else {
    ...
    Overlay::Overlay(
        ...
        dimensions{ pSpec->overlayWidth, 240 },
        pWindow{ MakeWindow_(pos_) },
        gfx{ pWindow->GetHandle() },
    ...
    void Overlay::UpdateDataSets_()
    {
        ...
        for (auto& w : pSpec->widgets) {
            try {
                if (auto pGraphSpec = std::get_if<GraphSpec>(&w)) {
                    for (const auto mi : pGraphSpec->metricIndices) {
                        auto pMetric = dynamic_cast<pmon::NumericMetric*>(pm->GetMetricByIndex(mi));
                        // TODO: silent fail this by inserting "empty" data pack and logging 
                        assert(pMetric);
                        const auto [i, inserted] = graphPacks.emplace(mi,
    ...
    {
        if (const DimensionsI newDims = pRoot->GetElementDims(); newDims != dimensions)
        {
            dimensions = newDims;
            pWindow->Resize(dimensions);
            if (!pWindow->Standard()) {
                pWindow->Move(CalculateOverlayPosition_());
            }
            gfx.Resize(dimensions);
            position = position_;
        }
    ...
    Vec2I Overlay::CalculateOverlayPosition_() const
    {
        ...
        {
            x = targetRect.right - dimensions.width;
        }
        ...
        {
            y = targetRect.bottom - dimensions.height;
        }
    ...
    void Overlay::SetCaptureState(bool active, std::wstring path, std::wstring name)
    {
        if (active && !pWriter)
        {
            ...
            pm->FlushRawData();
            pWriter = { pm->MakeRawFrameDataWriter(std::move(fullPath)) };
        }
