...
PmNsmFrameData* StreamClient::ReadFrameByIdx(uint64_t frame_id) {
  ...
  if (shared_mem_view_->IsEmpty()) {
    LOG(ERROR) << "Shared mem is empty. No new data.";
    return data;
  }
...
PM_STATUS StreamClient::RecordFrame(PM_FRAME_DATA** out_frame_data) {
  ...
  if (data) {
    ...
    CopyFrameData(nsm_hdr->start_qpc, data, *out_frame_data);
...
void StreamClient::CopyFrameData(uint64_t start_qpc,
  ...
  dst_frame->ms_stalled_on_queue_full =
      QpcDeltaToMs(src_frame->present_event.INTC_Timer[INTC_TIMER_WAIT_IF_FULL]
                       .mAccumulatedTime,
                   GetQpcFrequency());
  dst_frame->ms_waiting_on_queue_sync = QpcDeltaToMs(
      src_frame->present_event.INTC_Timer[INTC_TIMER_WAIT_UNTIL_EMPTY_SYNC]
          .mAccumulatedTime,
      GetQpcFrequency());
  dst_frame->ms_waiting_on_queue_drain = QpcDeltaToMs(
      src_frame->present_event.INTC_Timer[INTC_TIMER_WAIT_UNTIL_EMPTY_DRAIN]
          .mAccumulatedTime,
      GetQpcFrequency());
  dst_frame->ms_waiting_on_fence = QpcDeltaToMs(
      src_frame->present_event.INTC_Timer[INTC_TIMER_WAIT_FOR_FENCE]
          .mAccumulatedTime,
      GetQpcFrequency());
  dst_frame->ms_waiting_on_fence_submission = QpcDeltaToMs(
      src_frame->present_event.INTC_Timer[INTC_TIMER_WAIT_UNTIL_FENCE_SUBMITTED]
          .mAccumulatedTime,
      GetQpcFrequency());
  dst_frame->ms_stalled_on_queue_empty =
      QpcDeltaToMs(src_frame->present_event.INTC_Timer[INTC_TIMER_WAIT_IF_EMPTY]
                       .mAccumulatedTime,
                   GetQpcFrequency());
  dst_frame->ms_between_producer_presents =
      QpcDeltaToMs(src_frame->present_event.INTC_ProducerPresentTime -
                       src_frame->present_event.last_producer_present_qpc,
                   GetQpcFrequency());
  dst_frame->ms_between_consumer_presents =
      QpcDeltaToMs(src_frame->present_event.INTC_ConsumerPresentTime -
                       src_frame->present_event.last_consumer_present_qpc,
                   GetQpcFrequency());
  dst_frame->ms_waiting_on_sync_object = QpcDeltaToMs(
      src_frame->present_event
          .INTC_Timers[INTC_GPU_TIMER_SYNC_TYPE_WAIT_SYNC_OBJECT_CPU],
      GetQpcFrequency());
  dst_frame->ms_waiting_on_query_data = QpcDeltaToMs(
      src_frame->present_event
          .INTC_Timers[INTC_GPU_TIMER_SYNC_TYPE_POLL_ON_QUERY_GET_DATA],
      GetQpcFrequency());
  dst_frame->ms_waiting_on_draw_time_compilation = QpcDeltaToMs(
      src_frame->present_event
          .INTC_Timers[INTC_GPU_TIMER_WAIT_FOR_COMPILATION_ON_DRAW],
      GetQpcFrequency());
  dst_frame->ms_waiting_on_create_time_compilation = QpcDeltaToMs(
      src_frame->present_event
          .INTC_Timers[INTC_GPU_TIMER_WAIT_FOR_COMPILATION_ON_CREATE],
      GetQpcFrequency());
  dst_frame->ms_in_make_resident = QpcDeltaToMs(
      src_frame->present_event
                       .MemoryResidency[DXGK_RESIDENCY_EVENT_MAKE_RESIDENT],
      GetQpcFrequency());
  dst_frame->ms_in_paging_packets =
      QpcDeltaToMs(src_frame->present_event
          .MemoryResidency[DXGK_RESIDENCY_EVENT_PAGING_QUEUE_PACKET],
                   GetQpcFrequency());

  // power telemetry
  dst_frame->gpu_power_w = src_frame->power_telemetry.gpu_power_w;
  dst_frame->gpu_sustained_power_limit_w =
      src_frame->power_telemetry.gpu_sustained_power_limit_w;
  dst_frame->gpu_voltage_v = src_frame->power_telemetry.gpu_voltage_v;
  dst_frame->gpu_frequency_mhz = src_frame->power_telemetry.gpu_frequency_mhz;
  dst_frame->gpu_temperature_c = src_frame->power_telemetry.gpu_temperature_c;
  dst_frame->gpu_utilization = src_frame->power_telemetry.gpu_utilization;
  dst_frame->gpu_render_compute_utilization =
      src_frame->power_telemetry.gpu_render_compute_utilization;
  dst_frame->gpu_media_utilization =
      src_frame->power_telemetry.gpu_media_utilization;

  dst_frame->vram_power_w = src_frame->power_telemetry.vram_power_w;
  dst_frame->vram_voltage_v = src_frame->power_telemetry.vram_voltage_v;
  dst_frame->vram_frequency_mhz = src_frame->power_telemetry.vram_frequency_mhz;
  dst_frame->vram_effective_frequency_gbs =
      src_frame->power_telemetry.vram_effective_frequency_gbps;
  dst_frame->vram_read_bandwidth_bps =
      src_frame->power_telemetry.vram_read_bandwidth_bps;
  dst_frame->vram_write_bandwidth_bps =
      src_frame->power_telemetry.vram_write_bandwidth_bps;
  dst_frame->vram_temperature_c = src_frame->power_telemetry.vram_temperature_c;

  for (int i = 0; i < MAX_PM_FAN_COUNT; i++) {
    dst_frame->fan_speed_rpm[i] = src_frame->power_telemetry.fan_speed_rpm[i];
  }

  for (int i = 0; i < MAX_PM_PSU_COUNT; i++) {
    dst_frame->psu_type[i] =
        TranslatePsuType(src_frame->power_telemetry.psu[i].psu_type);
    src_frame->power_telemetry.psu[i].psu_type;
    dst_frame->psu_power[i] = src_frame->power_telemetry.psu[i].psu_power;
    dst_frame->psu_voltage[i] = src_frame->power_telemetry.psu[i].psu_voltage;
  }

  // Gpu memory telemetry
  dst_frame->gpu_mem_total_size_b =
      src_frame->power_telemetry.gpu_mem_total_size_b;
  dst_frame->gpu_mem_used_b = src_frame->power_telemetry.gpu_mem_used_b;
  dst_frame->gpu_mem_max_bandwidth_bps =
      src_frame->power_telemetry.gpu_mem_max_bandwidth_bps;
  dst_frame->gpu_mem_read_bandwidth_bps =
      src_frame->power_telemetry.gpu_mem_read_bandwidth_bps;
  dst_frame->gpu_mem_write_bandwidth_bps =
      src_frame->power_telemetry.gpu_mem_write_bandwidth_bps;

  // Throttling flags
  dst_frame->gpu_power_limited = src_frame->power_telemetry.gpu_power_limited;
  dst_frame->gpu_temperature_limited =
      src_frame->power_telemetry.gpu_temperature_limited;
  dst_frame->gpu_current_limited =
      src_frame->power_telemetry.gpu_current_limited;
  dst_frame->gpu_voltage_limited =
      src_frame->power_telemetry.gpu_voltage_limited;
  dst_frame->gpu_utilization_limited =
      src_frame->power_telemetry.gpu_utilization_limited;

  dst_frame->vram_power_limited = src_frame->power_telemetry.vram_power_limited;
  dst_frame->vram_temperature_limited =
  ...
  dst_frame->vram_voltage_limited =
      src_frame->power_telemetry.vram_voltage_limited;
  dst_frame->vram_utilization_limited =
      src_frame->power_telemetry.vram_utilization_limited;

  // cpu telemetry - only available in INTERNAL builds
  dst_frame->cpu_utilization = src_frame->cpu_telemetry.cpu_utilization;
  dst_frame->cpu_power_w = src_frame->cpu_telemetry.cpu_power_w;
  dst_frame->cpu_power_limit_w = src_frame->cpu_telemetry.cpu_power_limit_w;
  dst_frame->cpu_temperature_c = src_frame->cpu_telemetry.cpu_temperature;
  dst_frame->cpu_frequency = src_frame->cpu_telemetry.cpu_frequency;
  dst_frame->cpu_bias = src_frame->cpu_telemetry.cpu_bias_weight;
  dst_frame->gpu_bias = src_frame->cpu_telemetry.gpu_bias_weight;
}
