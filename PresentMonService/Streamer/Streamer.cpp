...
void Streamer::WriteFrameData(uint32_t process_id, PmNsmFrameData* data) {
    if (data == NULL) {
...
void Streamer::CopyFromPresentMonPresentEvent(
    ...
    nsm_present_event->SeenINTCFramePacerInfo =
        present_event->SeenINTCFramePacerInfo;
    ...
    nsm_present_event->INTC_ConsumerPresentTime =
        present_event->INTC_ConsumerPresentTime;
    for (uint32_t i = 0; i < INTC_Timer::INTC_TIMER_COUNT; i++) {
      nsm_present_event->INTC_Timer[i].mStartTime =
          present_event->INTC_Timer[i].mStartTime;
      nsm_present_event->INTC_Timer[i].mAccumulatedTime =
          present_event->INTC_Timer[i].mAccumulatedTime;
      nsm_present_event->INTC_Timer[i].mStartCount =
          present_event->INTC_Timer[i].mStartCount;
    }
    // "-track_cpu_gpu_sync" and "track_shader_compilation"
    for (uint32_t i = 0; i < INTCGPUTimer::INTC_GPU_TIMER_COUNT; i++) {
      nsm_present_event->INTC_Timers[i] = present_event->INTC_Timers[i];
    }
    // "debug_frame_pacing"
    nsm_present_event->INTC_FrameID = present_event->INTC_FrameID;
    nsm_present_event->INTC_AppWorkStart = present_event->INTC_AppWorkStart;
    nsm_present_event->INTC_AppSimulationTime =
        present_event->INTC_AppSimulationTime;
    nsm_present_event->INTC_DriverWorkStart =
        present_event->INTC_DriverWorkStart;
    nsm_present_event->INTC_DriverWorkEnd = present_event->INTC_DriverWorkEnd;
    nsm_present_event->INTC_KernelDriverSubmitStart =
        present_event->INTC_KernelDriverSubmitStart;
    nsm_present_event->INTC_KernelDriverSubmitEnd =
        present_event->INTC_KernelDriverSubmitEnd;
    nsm_present_event->INTC_GPUStart = present_event->INTC_GPUStart;
    nsm_present_event->INTC_GPUEnd = present_event->INTC_GPUEnd;
    nsm_present_event->INTC_KernelDriverFenceReport =
        present_event->INTC_KernelDriverFenceReport;
    nsm_present_event->INTC_PresentAPICall = present_event->INTC_PresentAPICall;
    nsm_present_event->INTC_TargetFrameTime =
        present_event->INTC_TargetFrameTime;
    nsm_present_event->INTC_FlipReceivedTime =
        present_event->INTC_FlipReceivedTime;
    nsm_present_event->INTC_FlipReportTime = present_event->INTC_FlipReportTime;
    nsm_present_event->INTC_FlipProgrammingTime =
        present_event->INTC_FlipProgrammingTime;
    nsm_present_event->INTC_ActualFlipTime = present_event->INTC_ActualFlipTime;
...
void Streamer::ProcessPresentEvent(
    ...
    data.present_event.last_producer_present_qpc = last_producer_present_qpc;
    data.present_event.last_consumer_present_qpc = last_consumer_present_qpc;
...
bool Streamer::UpdateNSMAttachments(uint32_t process_id, int& ref_count) {
  ...
  if (iter != process_shared_mem_map_.end()) {
    ...
    if (iter->second->GetRefCount() > 1) {
      ...
      process_shared_mem_map_.erase(iter);
      ref_count = 0;
    }
    return true;
  }
...
bool Streamer::CreateNamedSharedMemory(DWORD process_id,
  ...
  if (iter == process_shared_mem_map_.end()) {
    auto nsm =
        std::make_unique<NamedSharedMem>(mapfile_name, nsm_size_in_bytes);
    if (nsm->IsNSMCreated()) {
