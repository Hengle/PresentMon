...
namespace p2c::client::util
{
    ...
    std::unique_ptr<kern::OverlaySpec> MakeOverlaySpec(CefRefPtr<CefValue> vSpec)
    {
        ...
        // style sheets
        {
            ...
            sheets.push_back(Stylesheet::Make({ {"$graph"}, {"$label-swatch"} }));
            ...
            sheets.push_back(Stylesheet::Make({ {"$graph"}, {"$body-right", "$value"}}));
            sheets.back()->InsertRaw<at::width>(gutterSize);
            sheets.back()->InsertRaw<at::paddingLeft>(gutterPadding);
            sheets.back()->InsertRaw<at::textAlignment>(at::make::Enum(gfx::prim::Alignment::Center));
            sheets.back()->InsertRaw<at::textSize>((double)traversedPref["graphFont"]["valueSize"]);
            ...
            // Readout container
            {
                ...
                // stretch alignment should make this unnecessary
                sheets.back()->InsertRaw<at::width>(450.);
            ...
            // Readouts
            {
                ...
                sheets.back()->InsertRaw<at::width>(34.);
                sheets.back()->InsertRaw<at::paddingLeft>(2.);
            ...
            for (size_t i = 0; i < widgetCount; i++)
            {
                ...
                if (widgetType == kern::WidgetType::Graph) {
                    ...
                    pSpec->widgets.push_back(kern::GraphSpec{
                        .metricIndices = widgetMetrics.PluckAs<size_t>("metricId"),
                        .type = type, .tag = tag,
                    });

                    {
                        ...
                        sheets.push_back(Stylesheet::Make({ { "$graph", "$line" }, {"$footer"} }));
                        ...
                        for (size_t iWidgetMetric = 0; iWidgetMetric < widgetMetricCount; iWidgetMetric++) {
                            ...
                            sheets.push_back(Stylesheet::Make({ { "$graph", tag }, { std::format("$metric-{}", iWidgetMetric) } }));
                            ...
                            sheets.back()->InsertRaw<at::widgetMetricAxisAffinity>(
                                at::make::Enum(axisAffinity)
                            );

                            if (axisAffinity == AxisAffinity::Right) {
                                sheets.push_back(Stylesheet::Make({ { "$graph", tag }, { std::format("$metric-label-{}", iWidgetMetric) } }));
                                sheets.back()->InsertRaw<at::flexJustification>(
                                    at::make::Enum(FlexJustification::End)
                                );
                            }
                        }

                        // multi-line right axis switching
                        if (hasRightAxis) {
                            sheets.push_back(Stylesheet::Make({ { "$graph", tag }, {"$body-right", "$value"} }));
                            sheets.back()->InsertRaw<at::display>(at::make::Enum(Display::None));
                        }
                        else {
                            sheets.push_back(Stylesheet::Make({ {"$graph", tag }, {"$body-right", "$vert-axis"} }));
                            sheets.back()->InsertRaw<at::display>(at::make::Enum(gfx::lay::Display::None));
                        }
                    }
                }
                    ...
                    pSpec->widgets.push_back(kern::ReadoutSpec{ Traverse(vReadout)["metrics"][size_t(0)]["metricId"]});
                }
                else {
