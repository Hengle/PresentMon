  ...
  <v-app>
    ...
    >
      ...
      <v-list nav>
        ...
        <v-list-item color="primary" :to="{name: 'hotkey-config'}">
          <v-list-item-content>
            <v-list-item-title>Hotkeys</v-list-item-title>
          </v-list-item-content>
        </v-list-item>
...
export default Vue.extend({
  ...
  methods: {
    async initPreferences() {
      try {
        ...
        PrefStore.parseAndReplaceRawPreferenceString(payload);
      }
      catch (e) {
        await PrefStore.writeAttribute({attr: 'tutorialFinished', val: false});
    ...
    async handleHotkeyFired(action: Action) {
      if (action === Action.BeginCapture) {
        if (this.pid !== null) {
          PrefStore.writeCapture(true);
        }
      } else if (action === Action.EndCapture) {
        PrefStore.writeCapture(false);
      } else if (action === Action.ToggleCapture) {
        if (this.pid !== null) {
          PrefStore.writeCapture(!this.capturing);
        }
      } else if (action === Action.ShowOverlay) {
        PrefStore.writeAttribute({attr: 'hideAlways', val: false});
      } else if (action === Action.HideOverlay) {
        PrefStore.writeAttribute({attr: 'hideAlways', val: true});
  ...
  computed: {
    ...
    inSettings(): boolean {
      return ['hotkey-config', 'overlay-config', 'metric-processing']
