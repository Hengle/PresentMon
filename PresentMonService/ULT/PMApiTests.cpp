...
bool CompareFrameData(const PM_FRAME_DATA &data1, const PM_FRAME_DATA &data2) {
  ...
  if (!fltCmp(data1.ms_waiting_on_queue_sync, data2.ms_waiting_on_queue_sync,
              kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.ms_waiting_on_queue_drain, data2.ms_waiting_on_queue_drain,
              kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.ms_waiting_on_fence, data2.ms_waiting_on_fence,
              kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.ms_waiting_on_fence_submission,
              data2.ms_waiting_on_fence_submission, kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.ms_stalled_on_queue_empty, data2.ms_stalled_on_queue_empty,
              kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.ms_between_producer_presents,
              data2.ms_between_producer_presents,
              kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.ms_between_consumer_presents,
              data2.ms_between_consumer_presents, kErrorTolerance)) {
    return false;
  }
  ...
  if (!fltCmp(data1.ms_waiting_on_sync_object, data2.ms_waiting_on_sync_object,
              kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.ms_waiting_on_query_data, data2.ms_waiting_on_query_data,
              kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.ms_waiting_on_draw_time_compilation,
              data2.ms_waiting_on_draw_time_compilation, kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.ms_waiting_on_create_time_compilation,
              data2.ms_waiting_on_create_time_compilation, kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.ms_in_make_resident, data2.ms_in_make_resident,
              kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.ms_in_paging_packets, data2.ms_in_paging_packets,
              kErrorTolerance)) {
    return false;
  }

  // Power Telemetry data
  if (!fltCmp(data1.gpu_power_w, data2.gpu_power_w, kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.gpu_power_w, data2.gpu_power_w, kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.gpu_sustained_power_limit_w,
              data2.gpu_sustained_power_limit_w, kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.gpu_voltage_v, data2.gpu_voltage_v, kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.gpu_frequency_mhz, data2.gpu_frequency_mhz,
              kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.gpu_temperature_c, data2.gpu_temperature_c,
              kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.gpu_utilization, data2.gpu_utilization, kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.gpu_render_compute_utilization,
              data2.gpu_render_compute_utilization, kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.gpu_media_utilization, data2.gpu_media_utilization,
              kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.vram_power_w, data2.vram_power_w, kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.vram_voltage_v, data2.vram_voltage_v, kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.vram_frequency_mhz, data2.vram_frequency_mhz,
              kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.vram_effective_frequency_gbs,
              data2.vram_effective_frequency_gbs, kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.vram_read_bandwidth_bps, data2.vram_read_bandwidth_bps,
              kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.vram_write_bandwidth_bps, data2.vram_write_bandwidth_bps,
              kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.vram_temperature_c, data2.vram_temperature_c,
              kErrorTolerance)) {
    return false;
  }
  if (data1.gpu_mem_total_size_b != data2.gpu_mem_total_size_b) {
    return false;
  }
  if (data1.gpu_mem_used_b != data2.gpu_mem_used_b) {
    return false;
  }
  if (data1.gpu_mem_max_bandwidth_bps != data2.gpu_mem_max_bandwidth_bps) {
    return false;
  }
  if (!fltCmp(data1.gpu_mem_read_bandwidth_bps,
              data2.gpu_mem_read_bandwidth_bps,
              kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.gpu_mem_write_bandwidth_bps,
              data2.gpu_mem_write_bandwidth_bps,
              kErrorTolerance)) {
    return false;
  }
  if (data1.gpu_power_limited != data2.gpu_power_limited) {
    return false;
  }
  if (data1.gpu_temperature_limited != data2.gpu_temperature_limited) {
    return false;
  }
  if (data1.gpu_current_limited != data2.gpu_current_limited) {
    return false;
  }
  if (data1.gpu_voltage_limited != data2.gpu_voltage_limited) {
    return false;
  }
  if (data1.gpu_utilization_limited != data2.gpu_utilization_limited) {
    return false;
  }
  if (data1.vram_power_limited != data2.vram_power_limited) {
    return false;
  }
  if (data1.vram_temperature_limited != data2.vram_temperature_limited) {
    return false;
  }
  if (data1.vram_current_limited != data2.vram_current_limited) {
    return false;
  }
  if (data1.vram_voltage_limited != data2.vram_voltage_limited) {
    return false;
  }
  if (data1.vram_utilization_limited != data2.vram_utilization_limited) {
    return false;
  }
  if (!fltCmp(data1.cpu_utilization, data2.cpu_utilization, kErrorTolerance)) {
    return false;
  }
  if (!fltCmp(data1.cpu_frequency, data2.cpu_frequency, kErrorTolerance)) {
    return false;
  }
...
TEST_F(PresentMonApiULT, TestFpsMetrics) {
  ...
  std::string nsm_name;
  {
    ...
    for (int i = 0; i <= frame_gen.GetNumFrames(); i++) {
      ...
      test_streamer.WriteFrameData(kPid, &frame);
    }
    ...
    EXPECT_TRUE(fltCmp(fps_data.cpu_fps.avg, calculated_fps_data[0].cpu_fps.avg,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data.cpu_fps.percentile_99,
                       calculated_fps_data[0].cpu_fps.percentile_99,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data.cpu_fps.percentile_95,
                       calculated_fps_data[0].cpu_fps.percentile_95,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data.cpu_fps.percentile_90,
                       calculated_fps_data[0].cpu_fps.percentile_90,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data.cpu_fps.high,
                       calculated_fps_data[0].cpu_fps.high, kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data.cpu_fps.low, calculated_fps_data[0].cpu_fps.low,
                       kErrorTolerance));

    EXPECT_TRUE(fltCmp(fps_data.display_fps.avg,
                       calculated_fps_data[0].display_fps.avg,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data.display_fps.percentile_99,
                       calculated_fps_data[0].display_fps.percentile_99,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data.display_fps.percentile_95,
                       calculated_fps_data[0].display_fps.percentile_95,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data.display_fps.percentile_90,
                       calculated_fps_data[0].display_fps.percentile_90,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data.display_fps.high,
                       calculated_fps_data[0].display_fps.high,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data.display_fps.low,
                       calculated_fps_data[0].display_fps.low,
                       kErrorTolerance));

    EXPECT_TRUE(fltCmp(fps_data.gpu_fps_avg, calculated_fps_data[0].gpu_fps_avg,
                       kErrorTolerance));
    ...
    EXPECT_TRUE((fps_data.num_dropped_frames ==
                 calculated_fps_data[0].num_dropped_frames));
  }
...
TEST_F(PresentMonApiULT, TestFpsMetricsMultiSwapChains) {
  ...
  for (uint32_t i = 0; i < num_swap_chains; i++) {
    EXPECT_TRUE(fltCmp(fps_data[i].cpu_fps.avg,
                       calculated_fps_data[i].cpu_fps.avg, kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data[i].cpu_fps.percentile_99,
                       calculated_fps_data[i].cpu_fps.percentile_99,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data[i].cpu_fps.percentile_95,
                       calculated_fps_data[i].cpu_fps.percentile_95,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data[i].cpu_fps.percentile_90,
                       calculated_fps_data[i].cpu_fps.percentile_90,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data[i].cpu_fps.high,
                       calculated_fps_data[i].cpu_fps.high, kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data[i].cpu_fps.low,
                       calculated_fps_data[i].cpu_fps.low, kErrorTolerance));

    EXPECT_TRUE(fltCmp(fps_data[i].display_fps.avg,
                       calculated_fps_data[i].display_fps.avg,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data[i].display_fps.percentile_99,
                       calculated_fps_data[i].display_fps.percentile_99,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data[i].display_fps.percentile_95,
                       calculated_fps_data[i].display_fps.percentile_95,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data[i].display_fps.percentile_90,
                       calculated_fps_data[i].display_fps.percentile_90,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data[i].display_fps.high,
                       calculated_fps_data[i].display_fps.high,
                       kErrorTolerance));
    EXPECT_TRUE(fltCmp(fps_data[i].display_fps.low,
                       calculated_fps_data[i].display_fps.low,
                       kErrorTolerance));

    EXPECT_TRUE(fltCmp(fps_data[i].gpu_fps_avg,
                       calculated_fps_data[i].gpu_fps_avg, kErrorTolerance));
    ...
    EXPECT_TRUE((fps_data[i].num_dropped_frames ==
                 calculated_fps_data[i].num_dropped_frames));
  }
}
...
TEST_F(PresentMonApiULT, TestGfxLatencyMultiSwapChains) {
  ...
  std::string nsm_name;
  {
    ...
    for (uint32_t i = 0; i < num_swap_chains; i++) {
      EXPECT_TRUE(fltCmp(gfx_latency_data[i].display_latency_ms.avg,
                         calculated_latency_data[i].display_latency_ms.avg,
                         kErrorTolerance));
      EXPECT_TRUE(
          fltCmp(gfx_latency_data[i].display_latency_ms.percentile_99,
                 calculated_latency_data[i].display_latency_ms.percentile_99,
                 kErrorTolerance));
      EXPECT_TRUE(
          fltCmp(gfx_latency_data[i].display_latency_ms.percentile_95,
                 calculated_latency_data[i].display_latency_ms.percentile_95,
                 kErrorTolerance));
      EXPECT_TRUE(
          fltCmp(gfx_latency_data[i].display_latency_ms.percentile_90,
                 calculated_latency_data[i].display_latency_ms.percentile_90,
                 kErrorTolerance));
      EXPECT_TRUE(fltCmp(gfx_latency_data[i].display_latency_ms.high,
                         calculated_latency_data[i].display_latency_ms.high,
                         kErrorTolerance));
      EXPECT_TRUE(fltCmp(gfx_latency_data[i].display_latency_ms.low,
                         calculated_latency_data[i].display_latency_ms.low,
                         kErrorTolerance));

      EXPECT_TRUE(fltCmp(gfx_latency_data[i].render_latency_ms.avg,
                         calculated_latency_data[i].render_latency_ms.avg,
                         kErrorTolerance));
      EXPECT_TRUE(
          fltCmp(gfx_latency_data[i].render_latency_ms.percentile_99,
                 calculated_latency_data[i].render_latency_ms.percentile_99,
                 kErrorTolerance));
      EXPECT_TRUE(
          fltCmp(gfx_latency_data[i].render_latency_ms.percentile_95,
                 calculated_latency_data[i].render_latency_ms.percentile_95,
                 kErrorTolerance));
      EXPECT_TRUE(
          fltCmp(gfx_latency_data[i].render_latency_ms.percentile_90,
                 calculated_latency_data[i].render_latency_ms.percentile_90,
                 kErrorTolerance));
      EXPECT_TRUE(fltCmp(gfx_latency_data[i].render_latency_ms.high,
                         calculated_latency_data[i].render_latency_ms.high,
                         kErrorTolerance));
      EXPECT_TRUE(fltCmp(gfx_latency_data[i].render_latency_ms.low,
                         calculated_latency_data[i].render_latency_ms.low,
                         kErrorTolerance));
    }
  }
}
...
TEST_F(PresentMonApiULT, TestGfxLatencyDroppedFrames) {
  ...
  EXPECT_TRUE(fltCmp(gfx_latency_data.display_latency_ms.avg,
                     ...
                     kErrorTolerance));
  EXPECT_TRUE(
      fltCmp(gfx_latency_data.display_latency_ms.percentile_99,
             calculated_latency_data[0].display_latency_ms.percentile_99,
             kErrorTolerance));
  EXPECT_TRUE(
      fltCmp(gfx_latency_data.display_latency_ms.percentile_95,
             calculated_latency_data[0].display_latency_ms.percentile_95,
             kErrorTolerance));
  EXPECT_TRUE(
      fltCmp(gfx_latency_data.display_latency_ms.percentile_90,
             calculated_latency_data[0].display_latency_ms.percentile_90,
             kErrorTolerance));
  EXPECT_TRUE(fltCmp(gfx_latency_data.display_latency_ms.high,
                     calculated_latency_data[0].display_latency_ms.high,
...
TEST_F(PresentMonApiULT, TestGpuMetrics) {
  ...
  frame_gen.CalculateGpuMetrics((uint32_t)frame_gen.GetNumFrames(),
                                 kRunWindowSize, calculated_gpu_data);
...
TEST_F(PresentMonApiULT, TestCpuMetrics) {
  ...
  frame_gen.CalculateCpuMetrics((uint32_t)frame_gen.GetNumFrames(),
                                 kRunWindowSize, calculated_cpu_data);
...
TEST_F(PresentMonApiULT, TestFrameCapture) {
  ...
  for (uint32_t i = 0; i <= ((uint32_t)frame_gen.GetNumFrames() - 1); i++) {
    EXPECT_TRUE(CompareFrameData(frame_data[i], frame_gen.GetPmFrameData(i)));
  }
