...
if not exist "%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\." (
    echo error: this script assumes WPT is installed in the default location.
    echo Install the WPT here, or modify the script to point to your location:
    echo     %%ProgramFiles^(x86^)%%\windows kits\10\windows performance toolkit
    exit /b 1
)
...
set TRACE_INTEL_GRAPHICS_D3D10=ad367e62-97ef-4b20-8235-e8ab49db0c23
...
SET TRACE_CS_PROVIDERS_MIN=%TRACE_UMD%+%TRACE_DXGI%+%TRACE_D3D11ON12%+%TRACE_D3D9ON12%+%TRACE_D3D12%+%TRACE_D3D11%+%TRACE_INTEL_GRAPHICS_D3D10%+%TRACE_D3D10LEVEL9%
...
SET TRACE_CS_STATE_MIN=%TRACE_UMD%+%TRACE_DXGI%+%TRACE_D3D11ON12%+%TRACE_D3D9ON12%+%TRACE_D3D12%+%TRACE_D3D11%+%TRACE_INTEL_GRAPHICS_D3D10%+%TRACE_D3D10LEVEL9%
...
REM
set TRACE_NT_NORMAL=%TRACE_NT_LIGHT%+PROFILE+MEMINFO+DPC+INTERRUPT -stackwalk @"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\gpuview\EventsForStackTrace.txt"
set TRACE_NT_VERBOSE=%TRACE_NT_LIGHT%+PROFILE+MEMINFO+SYSCALL+DPC+INTERRUPT+ALL_FAULTS -stackwalk @"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\gpuview\EventsForStackTrace.txt"
...
REM Start Longhaul
REM 
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -on %TRACE_NT_LONGHAUL% %TRACE_LARGE_BUFFERS% -f Kernel.etl
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -start DxcLogger -on %TRACE_DXC_LONGHAUL% %TRACE_LARGE_BUFFERS% -f DXC.etl
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -capturestate DxcLogger %TRACE_DXC_LONGHAUL%
set TLOG=LONGHAUL
goto EndIt
...
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -on %TRACE_NT_PROVIDER% %TRACE_LARGE_BUFFERS% -f Kernel.etl

if "%ECHO_INFO%" == "yes" echo Xperf -start CaptureState -on %TRACE_CS_PROVIDERS% %TRACE_LARGE_BUFFERS% -f CaptureState.etl
if "%ECHO_INFO%" == "yes" echo.
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -start CaptureState -on %TRACE_CS_PROVIDERS% %TRACE_LARGE_BUFFERS% -f CaptureState.etl

if "%ECHO_INFO%" == "yes" echo Xperf -capturestate CaptureState %TRACE_CS_STATE%
if "%ECHO_INFO%" == "yes" echo.
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -capturestate CaptureState %TRACE_CS_STATE%

if "%ECHO_INFO%" == "yes" echo Xperf -start SchedulingLog -on %SCHEDULER_LOG_STATE% %TRACE_LARGE_BUFFERS% -f SchedulingLog.etl
if "%ECHO_INFO%" == "yes" echo.
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -start SchedulingLog -on %SCHEDULER_LOG_STATE% %TRACE_LARGE_BUFFERS% -f SchedulingLog.etl

if "%ECHO_INFO%" == "yes" echo Xperf -capturestate SchedulingLog %SCHEDULER_LOG_STATE%
if "%ECHO_INFO%" == "yes" echo.
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -capturestate SchedulingLog %SCHEDULER_LOG_STATE%

if "%ECHO_INFO%" == "yes" echo Xperf -start NoCaptureState -on %TRACE_NOCS_PROVIDERS% %TRACE_LARGE_BUFFERS% -f NoCaptureState.etl
if "%ECHO_INFO%" == "yes" echo.
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -start NoCaptureState -on %TRACE_NOCS_PROVIDERS% %TRACE_LARGE_BUFFERS% -f NoCaptureState.etl
...
:StartVerbose
REM "%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -setprofint 1221
SET TRACE_NT_PROVIDER=%TRACE_NT_NORMAL%
...
:StopLonghaul
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -stop DxcLogger
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -stop
echo All loggers stopped, starting merge...
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -merge DXC.etl kernel.etl Merged.etl
echo Restoring Performance Counter Interval
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -setprofint
set TLOG=
goto EndIt
...
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -stop SchedulingLog

if "%ECHO_INFO%" == "yes" echo Xperf -stop CaptureState
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -stop CaptureState
if "%ECHO_INFO%" == "yes" echo Xperf -stop NoCaptureState
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -stop NoCaptureState
if "%ECHO_INFO%" == "yes" echo Xperf -stop
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -stop
echo All loggers stopped, starting merge...
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -merge Kernel.etl NoCaptureState.etl CaptureState.etl SchedulingLog.etl Merged.etl
echo Restoring Performance Counter Interval
"%ProgramFiles(x86)%\windows kits\10\windows performance toolkit\xperf" -setprofint
goto EndIt
...
set TRACE_D3D12=
set TRACE_D3D11=
set TRACE_INTEL_GRAPHICS_D3D10=
