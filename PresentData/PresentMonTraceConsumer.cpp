    ...
    bool supportsDxgkPresentEvent)
{
    ...
    if (contextIter != mContexts.end() && contextIter->second.mFrameDmaInfo == nullptr) {
        contextIter->second.mFrameDmaInfo = &mProcessFrameDmaInfo.emplace(hdr.ProcessId, PMTraceConsumer::FrameDmaInfo{}).first->second;
    }
...
void PMTraceConsumer::HandleDxgkQueueComplete(EVENT_HEADER const& hdr, uint32_t submitSequence)
{
    ...
    auto dmaIter = mProcessFrameDmaInfo.find(pEvent->ProcessId);
    if (dmaIter != mProcessFrameDmaInfo.end()) {
        auto frameDmaInfo = &dmaIter->second;
...
void PMTraceConsumer::HandleDXGKEvent(EVENT_RECORD* pEventRecord)
{
    ...
    if (mTrackGPU) {
        ...
        case Microsoft_Windows_DxgKrnl::Context_Start::Id:
        {
            ...
            FrameDmaInfo* frameDmaInfo = hdr.EventDescriptor.Id == Microsoft_Windows_DxgKrnl::Context_Start::Id
                ? &mProcessFrameDmaInfo.emplace(hdr.ProcessId, PMTraceConsumer::FrameDmaInfo{}).first->second
                : nullptr;
            ...
            context->mFrameDmaInfo = frameDmaInfo;
            context->mNode = node;
            return;
        }
