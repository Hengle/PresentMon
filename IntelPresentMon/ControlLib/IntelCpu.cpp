...
bool IntelCpu::Sample() noexcept {
  ...
  { 
    ...
    if ((SubmitIpfExecuteCommand(&command, nullptr, &response))) {
      ...
      SetTelemetryCapBit(CpuTelemetryCapBits::cpu_temperature);
    }
  }
  ...
  // sample cpu power limit
  {
    ...
    if ((SubmitIpfExecuteCommand(&command, nullptr, &response))) {
      ...
      SetTelemetryCapBit(CpuTelemetryCapBits::cpu_power_limit);
    }
  }
  ...
  // sample cpu power
  {
    ...
    if ((SubmitIpfExecuteCommand(&command, nullptr, &response))) {
      ...
      SetTelemetryCapBit(CpuTelemetryCapBits::cpu_power);
    }
  }
  ...
  // sample cpu/dg bias
  {
    ...
    if ((SubmitIpfExecuteCommand(&command, nullptr, &response))) {
      ...
      SetTelemetryCapBit(CpuTelemetryCapBits::cpu_gpu_bias_weights);
    }
  }
