...
void StreamClient::CopyFrameData(uint64_t start_qpc,
  ...
  dst_frame->cpu_bias.data = src_frame->cpu_telemetry.cpu_bias_weight;
  dst_frame->cpu_bias.valid = cpu_telemetry_cap_bits[static_cast<size_t>(
      CpuTelemetryCapBits::cpu_gpu_bias_weights)];

  dst_frame->gpu_bias.data = src_frame->cpu_telemetry.gpu_bias_weight;
  dst_frame->gpu_bias.valid = cpu_telemetry_cap_bits[static_cast<size_t>(
      CpuTelemetryCapBits::cpu_gpu_bias_weights)];
}
